...
Socket::Socket(Socket&& other) noexcept : socket_(std::move(other.socket_)) { other.socket_ = INVALID_SOCKET; }
...
bool Socket::is_valid() const { return socket_ != INVALID_SOCKET; }

SOCKET Socket::get_handle() const { return socket_; }
...
size_t Socket::receive(void* buffer, size_t sz) {
    ...
    if (received == SOCKET_ERROR) throw NetworkException("Receive failed");
    return received;
}
